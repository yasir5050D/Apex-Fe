...
const RegistrationForm: React.FC = () => {
  ...
  const verifyPaymentStatus = async (orderId: string): Promise<boolean> => {
    try {
      const res = await fetch(`http://localhost:5000/api/payments/verify/${orderId}`);
  ...
  const updatePaymentStatus = async (orderId: string, status: string) => {
    try {
      const res = await fetch(`http://localhost:5000/api/payments/update-status`, {
        method: "POST",
    ...
    checkoutFn({
      ...
      onSuccess: async () => {
        ...
        if (verified) {
          ...
          Swal.close();
          if (updated)
            showSuccessMessage(`${formData.firstName} ${formData.lastName}`, orderId);
    ...
    <div className="max-w-xl mx-auto bg-white p-6 rounded-2xl shadow-lg mt-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <InputField id="lastName" label="Last Name" value={formData.lastName}
            onChange={(e) => setFormData({ ...formData, lastName: e.target.value })}
            error={errors.lastName} required />
        </div>

        <InputField id="parentage" label="Parentage" value={formData.parentage}
          onChange={(e) => setFormData({ ...formData, parentage: e.target.value })}
          error={errors.parentage} required />

        <div className="grid md:grid-cols-2 gap-4">
          <InputField id="email" label="Email" type="email" value={formData.email}
            onChange={(e) => setFormData({ ...formData, email: e.target.value })}
            error={errors.email} required />
          <InputField id="mobile" label="Mobile" type="tel" maxLength={10} value={formData.mobile}
        ...
        <TextareaField id="address" label="Full Address" value={formData.address}
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          >
            {loading ? <><SpinnerIcon className="animate-spin mr-2 h-5 w-5" /> Processing...</> : "Register & Pay"}
          </button>
      ...
      {!widgetReady && <p className="text-center text-gray-500 text-sm mt-3">⏳ Loading SMEPay widget...</p>}
    </div>
  );
};
